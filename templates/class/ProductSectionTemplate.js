...
export class ProductSectionListParser extends HoBomHTMLParserBase {
    constructor() {
        ...
        const { get } = this.$.http("https://fakestoreapi.com/");

        get("/products").then((datas) => {
            for (const product of datas) {
                ...
                this.$.createElementFromString(
                    ...
                    <div class="product-item" style="cursor: pointer;">
                      ...
                      <p style="font-weight: bold; margin-bottom: 1rem;">${product.title}</p>
                      <p>${product.description.slice(0, 30)}</p>
                      <p style="margin-top: 10px; font-weight: bold;">Price: $${product.price}</p>
                    </div>
                `,
                );
